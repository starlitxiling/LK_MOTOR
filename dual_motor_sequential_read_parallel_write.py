...
motor1 = LkMotor("/dev/tty.usbmodem01234567895", motor_id=2)
motor2 = LkMotor("/dev/tty.usbmodem01234567893", motor_id=1)
...
def motor_set_torque_worker(motor_instance, torque_value):
    """在线程中执行电机的set_torque_nm方法"""
    motor_instance.set_torque_nm(torque_value)

print("开始双电机互控 (串行读取，并行写入版)...")
try:
    while True:
        ...
        try:
            ...
            print(f"{timestamp} [错误] motor1.refresh() 失败: {e}")
        try:
            motor2.refresh()
        except Exception as e:
            print(f"{timestamp} [错误] motor2.refresh() 失败: {e}")
